...
window.t_forms__lang = (window.navigator.userLanguage || window.navigator.language).toUpperCase().slice(0, 2), window.scriptSysPayment = {}, window.handlerSysPayment = {}, window.isInitEventsZB = {}, window.isInitEventsCustomMask = {}, window.initForms = {}, window.tildaForm = { versionLib: "02.001", endpoint: "forms.tildacdn.com", isRecaptchaScriptInit: !1, currentFormProccessing: !1 }, t_forms__onReady(function() {
    ...
    var fileName, script, lang = window.t_forms__lang;
    "RU" !== lang && "EN" !== lang && (fileName = "tilda-forms-dict-1.0", document.head.querySelector('script[src*="' + fileName + '"]') || ((script = document.createElement("script")).type = "text/javascript", script.src = "https://static.tildacdn.com/js/" + fileName + ".min.js", script.onerror = function() { console.error("Failed to load tilda-forms-dictionary: ", this.src) }, document.head.appendChild(script)))
    ...
    for (var isMinOrderSetted, isMinQuantitySetted, inputs = form.querySelectorAll(".js-tilda-rule"), arrError = [], isEmptyValue = !0, i = 0; i < inputs.length; i++) {
        ...
        objError.obj = input, objError.type = [], value && value.length && (valueNoSpace = value.replace(/[\s\u0000—\u001F\u2000-\u200F\uFEFF\u2028-\u202F\u205F-\u206F]/gi, ""), value = value.trim()), 0 < value.length && (isEmptyValue = !1), minLength = minLength && parseInt(minLength, 10), maxLength = maxLength && parseInt(maxLength, 10);
    ...
    for (var inputBoxClassName = (inputBoxClassName = form.getAttribute("data-inputbox")) || ".blockinput", inputGroup = "", isErrorBox = !1, errorItem = "", errorInputs = "", strError = "", i = 0; i < arrErrors.length; i++)
        if (arrErrors[i] && arrErrors[i].obj) {
            if (0 === i && "none" === arrErrors[i].obj) { errorItem = form.querySelectorAll(".js-rule-error-all"); for (var j = 0; j < errorItem.length; j++) errorItem[j].innerHTML = t_forms__getMsg("emptyfill"), errorItem[j].style.display = "block"; break }
            var el = arrErrors[i].obj;
            el instanceof Element || (el = el[0]), el && (inputGroup = el.closest(inputBoxClassName)), inputGroup && (errorInputs = inputGroup.querySelectorAll(".t-input-error"), t_addClass(inputGroup, "js-error-control-box"), 0 < errorInputs.length && (isErrorBox = !0));
            for (j = 0; j < arrErrors[i].type.length; j++) {
                var error = arrErrors[i].type[j],
                    strError = "";
                if (1, (errorItem = form.querySelectorAll(".js-rule-error-" + error)).length)
                    for (var k = 0; k < errorItem.length; k++) errorItem[k].getAttribute("data-rule-filled") || (errorItem[k].textContent && errorItem[k].innerText || !t_forms__getMsg(error) ? strError = errorItem[0].textContent || errorItem[0].innerText : errorItem[k].innerHTML = t_forms__getMsg(error)), errorItem[k].style.display = "block";
                ...
                isErrorBox && (!strError && t_forms__getMsg(error + "field") ? strError = t_forms__getMsg(error + "field") : !strError && t_forms__getMsg(error) && (strError = t_forms__getMsg(error)), strError && inputGroup && (errorInputs = inputGroup.querySelectorAll(".t-input-error"), Array.prototype.forEach.call(errorInputs, function(input) { input.innerHTML = strError })))
            }
        }
    var errorBoxes = form.querySelectorAll(".js-errorbox-all");
    return Array.prototype.forEach.call(errorBoxes, function(error) { error.style.display = "block" }), t_triggerEvent(form, "tildaform:aftererror"), !0
}, window.tildaForm.addTildaCaptcha = function(form, strCaptcha) {
    form instanceof Element || (form = form[0]);
    var randomKey, formCaptchaBox = document.getElementById("tildaformcaptchabox"),
    ...
    try { randomKey = (new Date).getTime() + "=" + parseInt(8 * Math.random(), 10) } catch (e) { randomKey = "rnd=" + parseInt(8 * Math.random(), 10) }
    strCaptcha = '<div id="tildaformcaptchabox" style="z-index: 99999999999; position:fixed; text-align: center; vertical-align: middle; top: 0px; left:0px; bottom: 0px; right: 0px; background: rgba(255,255,255,0.97);"><iframe id="captchaIframeBox" src="//' + window.tildaForm.endpoint + "/procces/captcha/?tildaspec-formid=" + form.getAttribute("id") + "&tildaspec-formskey=" + strCaptcha + "&" + randomKey + '" frameborder="0" width="100%" height="100%"></iframe></div>';
    ...
    } catch (error) { return console.log("addMebersInfoToForm exception: ", error), !1 }
    return !0
    ...
    var bodyScrollTop, body = document.body;
    t_hasClass(body, "t-body_scroll-locked") || (bodyScrollTop = void 0 !== window.pageYOffset ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop, t_addClass(body, "t-body_scroll-locked"), body.style.top = "-" + bodyScrollTop + "px", body.setAttribute("data-popup-scrolltop", bodyScrollTop))
}, window.tildaForm.unlockBodyScroll = function() {
    var bodyScrollTop, body = document.body;
    t_hasClass(body, "t-body_scroll-locked") && (bodyScrollTop = body.getAttribute("data-popup-scrolltop"), t_removeClass(body, "t-body_scroll-locked"), body.style.top = null, body.removeAttribute("data-popup-scrolltop"), document.documentElement.scrollTop = parseInt(bodyScrollTop))
    ...
    try { window.mauser && window.tildaForm.addMebersInfoToForm(form) } catch (error) {}
    ...
    if (inputItsGood || form.insertAdjacentHTML("beforeend", '<div style="position: absolute; left: -5000px; bottom: 0; display: none;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments" tabindex="-1" /></div>'), 2 === formType || !formType && formKey) {
        var tildaSpecs = { "tildaspec-cookie": document.cookie, "tildaspec-referer": window.location.href, "tildaspec-formid": formId, "tildaspec-formskey": formKey, "tildaspec-version-lib": window.tildaForm.versionLib, "tildaspec-pageid": pageId, "tildaspec-projectid": projectId, "tildaspec-lang": window.t_forms__lang };
        ...
        var formUrl = "send.php",
            dataForm = [],
            arrFilter = [];
        if (dataForm = t_serializeArray(form), Array.prototype.forEach.call(dataForm, function(data) {-1 === data.name.indexOf("tildadelivery-") && arrFilter.push(data) }), dataForm = arrFilter, window.tildaForm.tildapayment && window.tildaForm.tildapayment.products) dataForm.push({ name: "tildapayment", value: JSON.stringify(window.tildaForm.tildapayment) });
            ...
            if (4 === xhr.readyState)
                if (200 <= xhr.status && xhr.status < 400) {
                    ...
                    if (data) {
                        ...
                        if ("object" == typeof objData) {
                            ...
                            if (t_removeClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "0", objData && objData.error) {
                                ...
                                Array.prototype.forEach.call(allError, function(error) { error.style.display = "block" }), Array.prototype.forEach.call(errorBoxes, function(error) { error.innerHTML = objData.error, error.style.display = "block" }), t_addClass(form, "js-send-form-error"), t_triggerEvent(form, "tildaform:aftererror")
                            } else {
                                ...
                                if (window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics) {
                                    ...
                                    try { window.Tilda.sendEventToStatistics(dataEventName, title, product, price) } catch (error) { console.log("Error while sending statistics. Code status: ", error) }
                                    window.dataLayer && window.dataLayer.push({ event: "submit_" + formId })
                                } else {
                                    try { ga && "tilda" !== window.mainTracker && ga("send", { hitType: "pageview", page: dataEventName, title: title }), window.mainMetrika && window[window.mainMetrika] && window[window.mainMetrika].hit(dataEventName, { title: title, referer: window.location.href }) } catch (error) { console.log("Error while sending main metrica. Code status: ", error) }
                                    window.dataLayer && window.dataLayer.push({ event: "submit_" + formId })
                                }
                                try { t_triggerEvent(form, "tildaform:aftersuccess"), dataFormSendedCallback && "function" == typeof jQuery ? eval(dataFormSendedCallback + "($(form));") : dataFormSendedCallback && eval(dataFormSendedCallback + "(form);") } catch (error) { console.log("Error while call success callback. Code status: ", error) }
                    ...
                    var msgContainers = t_forms__getErrorContainers(form, ""),
                        errorBoxes = msgContainers.errorBoxes,
                        allError = msgContainers.allError;
                    if (!xhr || 0 == xhr.status && tsDelta < 100) Array.prototype.forEach.call(allError, function(error) { error.innerHTML = "Request error (sending form data). Please check internet connection..." });
                    else {
                        ...
                        xhr && xhr.responseText ? Array.prototype.forEach.call(allError, function(error) { error.innerHTML = "[" + xhr.status + "] " + xhr.responseText + ". Please, try again later." }) : xhr && xhr.statusText ? Array.prototype.forEach.call(allError, function(error) { error.innerHTML = "Error [" + xhr.status + ", " + xhr.statusText + "]. Please, try again later." }) : Array.prototype.forEach.call(allError, function(error) { error.innerHTML = "[" + xhr.status + "] Unknown error. Please, try again later." })
                    }
                    Array.prototype.forEach.call(allError, function(error) { error.style.display = "block" }), Array.prototype.forEach.call(errorBoxes, function(error) { error.style.display = "block" }), t_addClass(form, "js-send-form-error"), t_triggerEvent(form, "tildaform:aftererror")
                }
        }, xhr.send(dataForm), !1
    }
    ...
    var dataForm = {};
    dataForm = t_serializeArray(form), window.tildaForm.tildapayment && window.tildaForm.tildapayment.amount && dataForm.push({ name: "tildapayment", value: JSON.stringify(window.tildaForm.tildapayment) }), dataForm = t_forms__formData(dataForm);
    var xhr = new XMLHttpRequest;
...
function t_forms__initForms() {
    ...
    window.t_forms__inputData = {}, t_forms__addRecaptcha(), Array.prototype.forEach.call(recBlocks, function(rec) {
        var recid = rec.id;
...
function t_forms__initEventPlaceholder(rec) {
    var eventFocus = "focus",
        eventBlur = "blur";
    document.addEventListener || (eventFocus = "focusin", eventBlur = "focusout"), t_removeEventListener(rec, eventFocus, t_forms__removePlaceholder), t_addEventListener(rec, eventFocus, t_forms__removePlaceholder, !0), t_removeEventListener(rec, eventBlur, t_forms__addPlaceholder), t_addEventListener(rec, eventBlur, t_forms__addPlaceholder, !0)
}

function t_forms__removePlaceholder(inputId) { var strPlace, form, input = (inputId = inputId || window.event).target || inputId.srcElement; "INPUT" === input.tagName && (form = input.closest("[data-input-lid]"), strPlace = input.getAttribute("placeholder"), inputId = "", form ? inputId = form.getAttribute("data-input-lid") : (form = input.closest("form")) && (inputId = form.getAttribute("data-input-lid")), strPlace && inputId && (window.t_forms__inputData[inputId] = strPlace, input.setAttribute("placeholder", ""))) }
...
function t_forms__addInputItsGood(allForms) {
    ...
    Array.prototype.forEach.call(allForms, function(form) {
        ...
        "1" === formActionType || inputItsGood || form.insertAdjacentHTML("beforeend", '<div style="position: absolute; left: -5000px; bottom: 0; display: none;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments" tabindex="-1" /></div>')
    })
}

function t_forms__addAttrAction(allForms) {
    allForms = allForms.querySelectorAll(".js-form-proccess");
    Array.prototype.forEach.call(allForms, function(form) { "2" === form.getAttribute("data-formactiontype") && form.setAttribute("action", "#") })
}

function t_forms__onSubmit(allForms) {
    allForms = allForms.querySelectorAll(".js-form-proccess");
    Array.prototype.forEach.call(allForms, function(form) { t_removeEventListener(form, "submit", t_forms__submitEvent), t_addEventListener(form, "submit", t_forms__submitEvent) })
}

function t_forms__onClick(rec) { t_addEventListener(rec, "dblclick", t_forms__initBtnDblClick), t_removeEventListener(rec, "click", t_forms__initBtnClick), t_addEventListener(rec, "click", t_forms__initBtnClick) }

function t_forms__initBtnDblClick(event) { if (((event = event || window.event).target || event.srcElement).closest('[type="submit"]')) return event.preventDefault ? event.preventDefault() : event.returnValue = !1, !1 }
    ...
    if (btnSubmit) {
        ...
        if (form) {
            ...
            if (btnSubmit.tildaSendingStatus && (formKey = btnSubmit.tildaSendingStatus), !(formKey && 1 <= formKey || t_hasClass(btnSubmit, "t706__submit_disable"))) {
                if (t_addClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "1", window.tildaForm.hideErrors(form), formType = window.tildaForm.validate(form), window.tildaForm.showErrors(form, formType)) return t_removeClass(btnSubmit, "t-btn_sending"), void(btnSubmit.tildaSendingStatus = "0");
                ...
                if (form.querySelector(".g-recaptcha") && grecaptcha) {
                    ...
                    return captchaId ? grecaptcha.reset(captchaId) : (opts = { size: "invisible", sitekey: form.getAttribute("data-tilda-captchakey"), callback: window.tildaForm.captchaCallback }, captchaId = grecaptcha.render(formId + "recaptcha", opts), form.tildaCaptchaClientId = captchaId), void grecaptcha.execute(captchaId)
                }
...
function t_forms__onRender(rec) {!rec.querySelector(".t396") || (t_removeEventListener(rec, "render", t_forms__renderEvent), t_addEventListener(rec, "render", t_forms__renderEvent)) }

function t_forms__renderEvent() { t_forms__onSubmit(this) }

function t_forms__submitEvent(event) {
    var btnStatus, btnSubmit = event;
    event.target && (btnSubmit = event.target), btnSubmit && (btnStatus = "", (btnSubmit = btnSubmit.querySelector('[type="submit"]')) && btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus), btnStatus && "3" === btnStatus ? btnSubmit.tildaSendingStatus = "" : (btnSubmit && !t_hasClass(btnSubmit, "t706__submit_disable") && btnSubmit.click(), event.preventDefault ? event.preventDefault() : event.returnValue = !1))
}
...
function t_asyncLoad(input) {
    ...
    dataMask && !dataInit && (dataPlaceholder ? t_onFuncLoad("t_customMask__mask", function() { t_customMask__mask(input, dataMask, { placeholder: dataPlaceholder }) }) : t_onFuncLoad("t_customMask__mask", function() { t_customMask__mask(input, dataMask) }), input.setAttribute("data-tilda-mask-init", "1"))
}

function t_forms__getErrorContainers(form, innerText) {
    var errorBoxes = form.querySelectorAll(".js-errorbox-all"),
        allError = form.querySelectorAll(".js-errorbox-all .js-rule-error-all");
    return errorBoxes.length || (form.insertAdjacentHTML("afterbegin", '<div class="js-errorbox-all"></div>'), errorBoxes = form.querySelectorAll(".js-errorbox-all")), allError.length || (Array.prototype.forEach.call(errorBoxes, function(error) { error.insertAdjacentHTML("beforeend", '<p class="js-rule-error-all">' + innerText + "</p>") }), allError = form.querySelectorAll(".js-errorbox-all .js-rule-error-all")), { errorBoxes: errorBoxes, allError: allError }
}
...
function t_forms__addRecaptcha() {
    ...
    Array.prototype.forEach.call(allRecaptchas, function(recaptcha) {
        var formId, head, script, captchaKey = recaptcha.getAttribute("data-tilda-captchakey");
        captchaKey ? (formId = recaptcha.getAttribute("id"), window.tildaForm.isRecaptchaScriptInit || (head = document.head, script = document.createElement("script"), window.tildaForm.isRecaptchaScriptInit = !0, script.type = "text/javascript", script.src = "https://www.google.com/recaptcha/api.js?render=explicit", script.async = !0, head.appendChild(script), head.insertAdjacentHTML("beforeend", '<style type="text/css">.js-send-form-success .grecaptcha-badge {display: none;}</style>')), document.getElementById(formId + "recaptcha") || recaptcha.insertAdjacentHTML("beforeend", '<div id="' + formId + 'recaptcha" class="g-recaptcha" data-sitekey="' + captchaKey + '" data-callback="window.tildaForm.captchaCallback" data-size="invisible"></div>')) : t_removeClass(recaptcha, "js-tilda-captcha")
    })
}
...
function t_parseScripts(main, scripts) {
    ...
    Array.prototype.forEach.call(scripts, function(oldScript) {
        ...
        oldScript.innerHTML.length ? (newScript.appendChild(document.createTextNode(oldScript.innerHTML)), oldScript.parentNode.replaceChild(newScript, oldScript)) : ((script = document.createElement("script")).src = oldScript.attributes.src.value, main.appendChild(script), t_removeEl(oldScript))
    })
}
...
function t_forms__onSuccess(form) {
    ...
    if (successBoxOffset > window.scrollY || documentHeight - inputBottom < windowHeight - 100) inputWrapper.classList.add(inputsboxHiddenClassName), documentHeight < windowHeight && tildaLabel && setTimeout(function() { t_fadeOut(tildaLabel) }, 300);
    else {
        for (var i = 0; i < arrNotScroll.length; i++)
            if (arrNotScroll[i] == recType) { isScroll = !1; break }
        isScroll && t_forms__scrollBeginForm(target), setTimeout(function() { inputWrapper.classList.add(inputsboxHiddenClassName) }, 400)
    }
    var successUrl = form.getAttribute("data-success-url");
    successUrl && setTimeout(function() { window.location.href = successUrl }, 500), 835 != recType && 862 != recType || (prevBtn = prevBtn.querySelector(".t835__btn_prev")) && (prevBtn.style.display = "none")
}
...
function t_forms__scrollBeginForm(target) {
    ...
    document.body.setAttribute("data-scrollable", "true"),
        function t_forms__animateScroll() { currentTime += increment, window.scrollTo(0, function(currentTime) { return (currentTime /= duration / 2) < 1 ? change / 2 * currentTime * currentTime * currentTime + start : change / 2 * ((currentTime -= 2) * currentTime * currentTime + 2) + start }(currentTime)), currentTime < duration ? setTimeout(t_forms__animateScroll, increment) : document.body.removeAttribute("data-scrollable") }()
}

function t_ready(fn) { "loading" != document.readyState ? fn() : document.addEventListener ? document.addEventListener("DOMContentLoaded", fn) : document.attachEvent("onreadystatechange", function() { "loading" != document.readyState && fn() }) }

function t_removeEl(el) { el && el.parentNode && el.parentNode.removeChild(el) }
Array.prototype.some || (Array.prototype.some = function(fn) {
        ...
        for (var t = Object(this), len = t.length >>> 0, thisArg = 2 <= arguments.length ? arguments[1] : void 0, i = 0; i < len; i++)
            if (i in t && fn.call(thisArg, t[i], i, t)) return !0;
        return !1
    }),
    function(e) {
        var matches = e.matches || e.matchesSelector || e.webkitMatchesSelector || e.mozMatchesSelector || e.msMatchesSelector || e.oMatchesSelector;
        e.matches = e.matchesSelector = matches || function(selector) {
            var matches = document.querySelectorAll(selector),
                th = this;
            return Array.prototype.some.call(matches, function(e) { return e === th })
        }
    }(Element.prototype), Element.prototype.closest || (Element.prototype.closest = function(s) {
        for (var el = this; el && 1 === el.nodeType;) {
            if (Element.prototype.matches.call(el, s)) return el;
            el = el.parentElement || el.parentNode
        }
        return null
    }), String.prototype.trim || (String.prototype.trim = function() { return this.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, "") }), Array.prototype.indexOf || (Array.prototype.indexOf = function(searchElement, n) {
        "use strict";
        var k;
        ...
        n |= 0;
        if (len <= n) return -1;
        for (k = Math.max(0 <= n ? n : len - Math.abs(n), 0); k < len; k++)
...
function t_removeEventListener(el, eventName, callback) { el.removeEventListener ? el.removeEventListener(eventName, callback, !1) : el.detachEvent && t_forms__htmlEvents["on" + eventName] ? el.detachEvent("on" + eventName, callback) : el["on" + eventName] = null }

function t_addEventListener(el, eventName, callback, options) { el.addEventListener ? el.addEventListener(eventName, callback, options) : el.attachEvent && t_forms__htmlEvents["on" + eventName] ? el.attachEvent("on" + eventName, callback) : el["on" + eventName] = callback }
    ...
    for (var arr = [], elements = form.querySelectorAll("input, textarea, button, select"), i = 0; i < elements.length; i++)
        if (!(!elements[i].name || elements[i].disabled || -1 < ["file", "reset", "submit", "button"].indexOf(elements[i].type)))
            if ("select-multiple" !== elements[i].type) - 1 < ["checkbox", "radio"].indexOf(elements[i].type) && !elements[i].checked || arr.push({ name: elements[i].name, value: elements[i].value });
                ...
                for (var options = elements[i].options, j = 0; j < options.length; j++) options[j].selected && arr.push({ name: options[j].name, value: options[j].value });
    return arr
}

function t_addClass(el, className) { document.body.classList ? el.classList.add(className) : el.className += (el.className ? " " : "") + className }

function t_removeClass(el, className) { document.body.classList ? el.classList.remove(className) : el.className = el.className.replace(new RegExp("(^|\\s+)" + className + "(\\s+|$)"), " ").replace(/^\s+/, "").replace(/\s+$/, "") }

function t_hasClass(el, className) { return document.body.classList ? el.classList.contains(className) : new RegExp("(\\s|^)" + className + "(\\s|$)").test(el.className) }

function t_forms__formData(obj) { for (var data = "", i = 0; i < obj.length; i++) "" !== data && (data += "&"), data += encodeURIComponent(obj[i].name) + "=" + encodeURIComponent(obj[i].value); return data.replace(/%20/g, "+") }

function t_fadeOut(el) { var opacity, timer; "none" !== el.style.display && (opacity = 1, timer = setInterval(function() { el.style.opacity = opacity, (opacity -= .1) <= .1 && (clearInterval(timer), el.style.display = "none", el.style.opacity = null) }, 30)) }

function t_fadeIn(el) { var opacity, timer; "block" !== el.style.display && (opacity = 0, el.style.opacity = opacity, el.style.display = "block", timer = setInterval(function() { el.style.opacity = opacity, 1 <= (opacity += .1) && clearInterval(timer) }, 30)) }
