function t270_scroll(hash, offset, speed) {
    ...
    if (dontChangeHistory) { complete = function() {} }
...
function t396_init(recid) {
    ...
    $(window).resize(function() {
        ...
        t396_waitForFinalEvent(function() { if ($isMobile) { var ww = $(window).width(); if (ww != window.tn_window_width) { t396_doResize(recid) } } else { t396_doResize(recid) } }, 500, 'resizeruniqueid' + recid)
    });
    ...
    $(window).on('load', function() {
        ...
        if (typeof t_lazyload_update === 'function' && ab.css('overflow') === 'auto') { ab.bind('scroll', t_throttle(function() { if (window.lazy === 'y' || $('#allrecords').attr('data-tilda-lazy') === 'yes') { t_onFuncLoad('t_lazyload_update', function() { t_lazyload_update() }) } }, 500)) }
        if (window.location.hash !== '' && ab.css('overflow') === 'visible') {
            ab.css('overflow', 'hidden');
            setTimeout(function() { ab.css('overflow', 'visible') }, 1)
        }
    });
    var rec = $('#rec' + recid);
    ...
    rec.find('.t396__elem').each(function() {
        ...
        if (containerProp === 'grid') {
            if (isOnlyScalable) {
                ...
                $el.find('.tn-atom').wrap("<div class='tn-atom__scale-wrapper' style='transform: scale(" + window.tn_scale_factor + ");'></div>")
            } else {
...
function t396_doResize(recid) {
    ...
    var ww;
    var rec = $('#rec' + recid);
    if ($isMobile) { ww = $(window).width() } else { ww = window.innerWidth }
    var res = t396_detectResolution();
    rec.find('.t396__scale-style').remove();
    if (!isOnlyScalable) {
        rec.find('.t396__elem').css('zoom', '');
        rec.find('.t396__elem .tn-atom').css('transform-origin', '')
    } else { rec.find('.tn-atom').each(function() { if ($(this).closest('.tn-atom__scale-wrapper').length) { $(this).unwrap('.tn-atom__scale-wrapper') } }) }
    var ab = rec.find('.t396__artboard');
    var abWidth = ab.width();
...
function t396_updateTNobj() {
    ...
    if (typeof window.zero_window_width_hook != 'undefined' && window.zero_window_width_hook == 'allrecords' && $('#allrecords').length) { window.tn.window_width = parseInt($('#allrecords').width()) } else { window.tn.window_width = parseInt($(window).width()) }
...
function t396_switchResolution(res, resmax) {
    ...
    if (typeof resmax == 'undefined') { if (res == 1200) resmax = ''; if (res == 960) resmax = 1200; if (res == 640) resmax = 960; if (res == 480) resmax = 640; if (res == 320) resmax = 480 }
...
function t396_artboard_build(data, recid) {
    ...
    ab.find('.tn-elem').each(function() {
        var item = $(this);
        if (item.attr('data-elem-type') == 'text') { t396_addText(ab, item) }
    ...
    var overflowValue = ab.attr('data-artboard-ovrflw');
    if (overflowValue === 'visible' || overflowValue === 'visibleX') { $('#allrecords').css('overflow', 'hidden') }
    if ($isMobile) { $('#rec' + recid).append('<style>@media only screen and (min-width:1366px) and (orientation:landscape) and (-webkit-min-device-pixel-ratio:2) {.t396__carrier {background-attachment:scroll!important;}}</style>') }
}

function t396_ab__renderView(ab) {
    var fields = window.tn.ab_fields;
    for (var i = 0; i < fields.length; i++) { t396_ab__renderViewOneField(ab, fields[i]) }
    var ab_min_height = t396_ab__getFieldValue(ab, 'height');
    var ab_max_height = t396_ab__getHeight(ab);
    var isTildaModeEdit = $('#allrecords').attr('data-tilda-mode') == 'edit';
    var isScaled = t396_ab__getFieldValue(ab, 'upscale') === 'window';
    ...
    var offset_top = 0;
    if (ab_min_height == ab_max_height || (scaledMinHeight && scaledMinHeight >= ab_max_height)) { offset_top = 0 } else {
        var ab_valign = t396_ab__getFieldValue(ab, 'valign');
        if (ab_valign == 'top') { offset_top = 0 } else if (ab_valign == 'center') { if (scaledMinHeight) { offset_top = parseFloat((ab_max_height - scaledMinHeight) / 2).toFixed(1) } else { offset_top = parseFloat((ab_max_height - ab_min_height) / 2).toFixed(1) } } else if (ab_valign == 'bottom') { if (scaledMinHeight) { offset_top = parseFloat((ab_max_height - scaledMinHeight)).toFixed(1) } else { offset_top = parseFloat((ab_max_height - ab_min_height)).toFixed(1) } } else if (ab_valign == 'stretch') {
            offset_top = 0;
...
function t396_addImage(ab, el) {
    ...
    el.attr('data-fields', fields_str);
    t396_elem__renderView(el);
    el.find('img').on("load", function() { t396_elem__renderViewOneField(el, 'top'); if (typeof $(this).attr('src') != 'undefined' && $(this).attr('src') != '') { setTimeout(function() { t396_elem__renderViewOneField(el, 'top') }, 2000) } }).each(function() { if (this.complete) $(this).trigger('load') });
...
function t396_addVideo(ab, el) {
    ...
    if (typeof vihascover == 'undefined') { vihascover = '' }
    if (vihascover == 'y') {
    ...
    if (typeof viyid != 'undefined' && viyid != '') {
        ...
        if (vihascover == 'y') {
            ...
            if (autoplay == 'y' && mute == 'y' && window.lazy == 'y') { el.append('<script>lazyload_iframe = new LazyLoad({elements_selector: ".t-iframe"});<\/script>') }
            ...
            } else { viel.html('<iframe id="youtubeiframe" width="100%" height="100%" src="' + url + '" frameborder="0" allowfullscreen data-flag-inst="y"></iframe>') }
        }
    }
    ...
    if (typeof vivid != 'undefined' && vivid > 0) {
        ...
        if (vihascover == 'y') {
            ...
            if (window.lazy == 'y') {
                ...
                el.append('<script>lazyload_iframe = new LazyLoad({elements_selector: ".t-iframe"});<\/script>')
            } else { viel.html('<iframe src="' + url + '" width="100%" height="100%" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>') }
        }
    }
}
...
function t396_addTooltip(ab, el) {
    ...
    if (isMobile || (typeof tipopen != 'undefined' && tipopen == 'click')) { t396_setUpTooltip_mobile(el, pinEl, tipEl) } else { t396_setUpTooltip_desktop(el, pinEl, tipEl) }
...
function t396_elem__setFieldValue(el, prop, val, flag_render, flag_updateui, res) {
    ...
    if (res < 1200 && prop != 'zindex') { el.attr('data-field-' + prop + '-res-' + res + '-value', val) } else { el.attr('data-field-' + prop + '-value', val) }
...
function t396_elem__renderView(el) {
    ...
    if (!fields) { return !1 }
    fields = fields.split(',');
    for (var i = 0; i < fields.length; i++) { t396_elem__renderViewOneField(el, fields[i]) }
}
...
function t396_elem__convertPosition__Local__toAbsolute(el, field, value) {
    ...
    if (field == 'left') {
        ...
        if (el_leftunits === '%') { value = t396_roundFloat(el_container_width * value / 100) }
        ...
        if (isScaledElement && el_container !== 'window') {
            var elementWidth = t396_elem__getWidth(el);
    ...
    if (field === 'top') {
        ...
        var getElementHeight = function(el) {
            var height = t396_elem__getHeight(el);
            if (el.attr('data-elem-type') === 'image') {
                var width = t396_elem__getWidth(el);
        ...
        if (el_topunits === '%') { value = (el_container_height * (value / 100)) }
        ...
        if (isScaledElement && el_container !== 'window') {
            var elementHeight = getElementHeight(el);
...
function t396_ab__getHeight(ab, ab_height) {
    ...
    var ab_height_vh = t396_ab__getFieldValue(ab, 'height_vh');
    if (ab_height_vh != '') { ab_height_vh = parseFloat(ab_height_vh); if (isNaN(ab_height_vh) === !1) { var ab_height_vh_px = parseFloat(window.tn.window_height * parseFloat(ab_height_vh / 100)); if (ab_height < ab_height_vh_px) { ab_height = ab_height_vh_px } } }
    return (ab_height)
}

function t396_hex2rgb(hexStr) { var hex = parseInt(hexStr.substring(1), 16); var r = (hex & 0xff0000) >> 16; var g = (hex & 0x00ff00) >> 8; var b = hex & 0x0000ff; return [r, g, b] }
String.prototype.t396_replaceAll = function(search, replacement) { var target = this; return target.replace(new RegExp(search, 'g'), replacement) };
...
function t396_elem__getWidth(el, value) {
    ...
    if (el_widthunits == '%') { var el_container = t396_elem__getFieldValue(el, 'container'); if (el_container == 'window') { value = parseFloat(window.tn.window_width * parseFloat(parseInt(value) / 100)) } else { value = parseFloat(window.tn.grid_width * parseFloat(parseInt(value) / 100)) } }
    return (value)
}
...
function t396_elem__getHeight(el, value) {
    ...
    if (el.attr('data-elem-type') == 'shape' || el.attr('data-elem-type') == 'video' || el.attr('data-elem-type') == 'html' || el.attr('data-elem-type') == 'gallery') { var el_heightunits = t396_elem__getFieldValue(el, 'heightunits'); if (el_heightunits == '%') { var ab = el.parent(); var ab_min_height = parseFloat(ab.attr('data-artboard-proxy-min-height')); var ab_max_height = parseFloat(ab.attr('data-artboard-proxy-max-height')); var el_container = t396_elem__getFieldValue(el, 'container'); if (el_container == 'window') { value = parseFloat(ab_max_height * parseFloat(value / 100)) } else { value = parseFloat(ab_min_height * parseFloat(value / 100)) } } } else if (el.attr('data-elem-type') == 'button') { value = value } else { value = parseFloat(el.innerHeight()) }
    return (value)
}
...
function t396_setUpTooltip_desktop(el, pinEl, tipEl) {
    ...
    pinEl.mouseover(function() {
        $('.tn-atom__tip_visible').each(function() { var thisTipEl = $(this).parents('.t396__elem'); if (thisTipEl.attr('data-elem-id') != el.attr('data-elem-id')) { t396_hideTooltip(thisTipEl, $(this)) } });
        clearTimeout(timer);
        if (tipEl.css('display') == 'block') { return }
...
function t396_setUpTooltip_mobile(el, pinEl, tipEl) {
    pinEl.on('click', function(e) { if (tipEl.css('display') == 'block' && $(e.target).hasClass("tn-atom__pin")) { t396_hideTooltip(el, tipEl) } else { t396_showTooltip(el, tipEl) } });
    ...
    $(document).click(function(e) {
        ...
        if (isInsideTooltip) { var clickedPinId = $(e.target).parents(".t396__elem").attr("data-elem-id"); if (clickedPinId == id) { return } }
...
function t396_showTooltip(el, tipEl) {
    ...
    var padd = 15;
    if (pos == 'right' || pos == 'left') { var tipElRight = elRight + padd + tipElWidth; var tipElLeft = elLeft - padd - tipElWidth; if ((pos == 'right' && tipElRight > winWidth) || (pos == 'left' && tipElLeft < 0)) { pos = 'top' } }
